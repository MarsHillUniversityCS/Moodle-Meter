...
function meter_cron(){
    ...
    if(!$courses){
        mtrace('No courses found on which to run meter statistics');
    } else {
        foreach($courses as $course){ 
            ...
            mtrace('Doing stats run for course id '.$course->courseid.'...', '');
            do_cron_stats_run($course->courseid);
            mtrace('Done.');
        }
    }

    $DB->set_field('block', 'lastcron', time(), array('name'=>'meter'));
}
